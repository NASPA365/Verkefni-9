...
function renderResults(location, results) {
  ...
  const body = results.map(({ time, temperature, precipitation }) => {
    // Breyta ISO 8601 tíma í 24 tíma tímakerfi
    const date = new Date(time);
    const options = { hour: "2-digit", minute: "2-digit", hour12: false };
    const formattedTime = date.toLocaleTimeString("is-IS", options);

    return el(
      "tr",
      {},
      el("td", {}, formattedTime), // Sýnir tímann í 24 tíma
      el("td", {}, temperature.toFixed(1)), // Sýnir hitastig með einu aukastaf
      el("td", {}, precipitation.toFixed(1)) // Sýnir úrkomu
    );
  });

  const resultsTable = el("table", { class: "forecast" }, header, ...body);

  // Búa til header yfir veðurspá
  const headerResult = el("h2", {}, `Niðurstöður`);
  const locationHeader = el("h3", {}, `${location.title}`);
  const coordinatesInfo = el(
    "p",
    {},
  ...
  renderIntoResultsContent(
    el(
      ...
      {},
      headerResult,
      locationHeader, // Sýnir heiti staðar
      coordinatesInfo, // Sýnir breiddargráðu og lengdargráðu
      resultsTable // Sýnir veðurspá töfluna
    )
  );
}
...
async function onSearchMyLocation() {
  if (!navigator.geolocation) {
    renderError(new Error("Vafrinn styður ekki staðsetningar."));
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      try {
        ...
        renderResults(
          { title: "Þín staðsetning", lat: latitude, lng: longitude },
          results
        );
      } catch (error) {
        renderError(error);
    ...
    () => {
      renderError(
        new Error(
          "Ekki tókst að sækja staðsetningu, best væri að nota vafra: Chrome, Microsoft Edge, Firefox eða Safari"
        )
      );
    }
  );
